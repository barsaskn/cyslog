...
void parse_test() {
    char* test_packet = "<34>1 2024-05-16T12:00:00.000Z mymachine.example.com su - ID47 - BOM'su root' failed for lonvick on /dev/pts/8";
    Syslog_message* test_syslog_message =  parse_rfc5424(test_packet);
    assert(strcmp(test_syslog_message->priority, "<34>") == 0);
